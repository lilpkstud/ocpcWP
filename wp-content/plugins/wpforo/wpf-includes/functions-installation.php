...
function wpforo_activation(){
	...
	$args = array( 'table' => WPF()->tables->posts, 'col' => 'topicid_solved', 'check' => 'key_exists' );
	if( !wpforo_db_check( $args ) ) {
		@WPF()->db->query( "ALTER TABLE `".WPF()->tables->posts."` ADD KEY `topicid_solved` (`topicid`,`is_answer`)" );
	}
	$args = array( 'table' => WPF()->tables->topics, 'col' => 'forumid_status', 'check' => 'key_exists' );
	if( !wpforo_db_check( $args ) ) {
...
function wpforo_database_notice(){
    $url = admin_url('admin.php?page=wpforo-tools&tab=debug&view=tables');
    $class = 'wpforo-dbnote notice notice-warning is-dismissible';
    ...
    echo '<div class="' . $class . '" style="padding:15px 20px;"><h2 style="margin:0px;">' . esc_html($header) . ' </h2><p style="font-size:15px;margin:5px 0px;">' . $message . '</p><p style="margin:0px 0px 10px 0px;">' . $button . '</p><hr /><p style="margin:0px;color:#999; font-size:12px;">' . $info . '</p></div>';
}
