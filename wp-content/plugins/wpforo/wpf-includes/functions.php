...
function wpforo_get_request_uri($with_port = FALSE, $get_referer_when_ajax = TRUE){
    ...
    $url = $protocol . "://" . wpfval($_SERVER, 'HTTP_HOST') . ($with_port && $server_port ? $port : '') . wpfval($_SERVER, 'REQUEST_URI');
					...
					if(!empty($items)) :
						...
						if(!empty($items)) :
							foreach($items as $key => $item) : ?>
								...
								<tr id="post-<?php echo esc_attr(md5($item[$primary_key])) ?>" class="post-<?php echo esc_attr(md5($item[$primary_key])) ?> type-page status-publish hentry <?php echo ($key % 2 == 0) ? 'alternate' : ''  ?> iedit author-self" valign="top">
									...
									<?php $i = 0; foreach($fields  as $field) :
										if($i == 0) : ?>
											...
											<td>
												<strong>
													<a href="<?php echo esc_url($url) ?>" title="Edit “<?php echo esc_attr($varname) ?>”">
														<?php
															if($varname == 'forum'){
																$depth = 0;
 ...
 * @return null|mixed
 */
function wpfval( $array, $a, $b = null, $c = null ){
    if( wpfkey($array, $a, $b, $c) ){
        if( $b || $b === 0 ){
            if( $c || $c === 0 ) return $array[$a][$b][$c];
            return $array[$a][$b];
        }
        return $array[$a];
    }
 ...
 * @return bool
 */
function wpfkey( $array, $a, $b = null, $c = null ){
    if($a || $a === 0){
        if( is_array($array) && array_key_exists($a, $array) ){
            if($b || $b === 0){
                if( is_array($array[$a]) && array_key_exists($b, $array[$a]) ){
                    if($c || $c === 0){
                        if( is_array($array[$a][$b]) && array_key_exists($c, $array[$a][$b]) ) return true;
                    }else{
                        return true;
                    }
                }
